...
df2 <- read.csv(unz(filename, "household_power_consumption.txt"), header=T,
...
df2$timestamp = strptime(paste(df2$Date, df2$Time),format="%d/%m/%Y %H:%M:%S", tz="UTC")
...
plot(df2$timestamp, df2$Global_active_power, type="l", xlab="",
