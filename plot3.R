# Downloading the file

filename = "exdata-data-household_power_consumption.zip"

if (!file.exists(filename)) {
  retval = download.file("https://d396qusza40orc.cloudfront.net/exdata%2Fdata%2Fhousehold_power_consumption.zip",
                         destfile = filename,
                         method = "curl")
} 

# Reading the data from downloaded file

df3 <- read.csv(unz(filename, "household_power_consumption.txt"), header=T,
                sep=";", stringsAsFactors=F, na.strings="?",
                colClasses=c("character", "character", "numeric",
                             "numeric", "numeric", "numeric",
                             "numeric", "numeric", "numeric"))

# Format the date 

df3$timestamp = strptime(paste(df3$Date, df3$Time),format="%d/%m/%Y %H:%M:%S", tz="UTC")

# Format the start time and end time
startTime = strptime("01/02/2007 00:00:00", format="%d/%m/%Y %H:%M:%S", tz="UTC")
endTime = strptime("02/02/2007 23:59:59", format="%d/%m/%Y %H:%M:%S", tz="UTC")

df3 = df3[df3$timestamp >= startTime & df3$timestamp <= endTime , ]

#Construct the plot and save it to a PNG file with a width of 480 pixels and a height of 480 pixels.
png(filename="plot3.png", width=480, height=480)

plot(df3$timestamp, df3$Sub_metering_1, type="l", xlab="",
     ylab="Energy sub metering")
lines(df3$timestamp, df3$Sub_metering_2, col="red")
lines(df3$timestamp, df3$Sub_metering_3, col="blue")
legend("topright", legend=c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"),
       col=c("black", "red", "blue"), lwd=par("lwd"))
dev.off()
